    ...
    window.DocTireIndexPage = {
        ...
        init() {
            ...
            this.tableBody?.addEventListener('click', (event) => {
                ...
                const { action, docentry, technicianUserId } = buttonElement.dataset;
                this.handleAction(action, Number(docentry || 0), technicianUserId || null);
            });

            // Status toggle filters
            this.currentFilter = { status: '', page: 1, perPage: 10 };
            const toggleContainer = document.getElementById('doc-tire-status-toggles');
            if (toggleContainer) {
        ...
        renderRow(row) {
            ...
            // Liberar (solo para PLANEADOS)
            if (status === 'PLANNED') {
                const techId = row.technician_user_id ? `data-technician-user-id="${row.technician_user_id}"` : '';
                visibleBtns += button('btn-outline-primary', 'fa-paper-plane', '', 'release', docentry, techId);
            }
        ...
        async handleAction(action, docentry, extraData = null) {
            switch (action) {
                case 'edit':
                ...
                case 'send-mail':
                    ...
                    return;
                case 'release':
                    await this.releaseDocument(docentry, extraData);
                    return;
                case 'close':
        ...
        async releaseDocument(docentry, currentTechnicianUserId) {
            // Obtener opciones de mecánicos (empleados con position_code=MECHANIC, activos, con user_id)
            var mechanicOptions = [];
            if (this.routes.getFormOptions) {
                try {
                    var resp = await fetch(this.routes.getFormOptions, {
                        headers: { 'X-Requested-With': 'XMLHttpRequest' }
                    });
                    var data = await resp.json();
                    if (data.Success === 'Ok' && Array.isArray(data.Data?.mechanic_options)) {
                        mechanicOptions = data.Data.mechanic_options;
                    }
                } catch (e) {
                    console.warn('No se pudieron cargar opciones de técnicos:', e);
                }
            }

            // Mapear a {value, label} con user_id como valor y nombre completo como etiqueta
            var currentTechLabel = '';
            var techOptionsHtml = '<option value="">Seleccione un técnico...</option>';
            mechanicOptions.forEach(function (emp) {
                var val = emp.user_id ? String(emp.user_id) : '';
                var label = [emp.first_name, emp.last_name].filter(Boolean).join(' ') || val || '(sin nombre)';
                if (!val) return;
                var selected = '';
                if (currentTechnicianUserId && String(val) === String(currentTechnicianUserId)) {
                    selected = ' selected';
                    currentTechLabel = label;
                }
                techOptionsHtml += '<option value="' + val + '"' + selected + '>' + label + '</option>';
            });

            var messageHtml = '<p>Seleccione el técnico responsable para esta orden:</p>' +
                '<select id="swal-release-technician" class="form-select">' + techOptionsHtml + '</select>';
            if (currentTechLabel) {
                messageHtml = '<p class="mb-2">Técnico actual: <strong>' + currentTechLabel + '</strong></p>' +
                    '<p>Seleccione el técnico responsable para esta orden:</p>' +
                    '<select id="swal-release-technician" class="form-select">' + techOptionsHtml + '</select>';
            }
            ...
            var result = await Swal.fire({
                ...
                preConfirm: function () {
                    var techId = document.getElementById('swal-release-technician').value;
                    if (!techId) {
                        Swal.showValidationMessage('Debe seleccionar un técnico');
                        return false;
                    }
                    return techId;
                }
            ...
            var technicianUserId = result.value;

            const formData = new FormData();
            formData.append('_csrf', csrfToken());
            formData.append('docentry', String(docentry));
            formData.append('technician_user_id', String(technicianUserId));

            try {
                const response = await fetch(this.routes.release, {
                    method: 'POST',
                    headers: { 'X-Requested-With': 'XMLHttpRequest' },
                    body: formData,
                });
                const payload = await response.json();
                if (payload.Success !== 'Ok') {
                    throw new Error(payload.Msg || 'No se pudo liberar la orden');
                }

                Swal.fire('Liberada', payload.Msg || 'Orden liberada correctamente', 'success');
                this.currentFilter.page = 1;
                this.loadList();
            } catch (error) {
                Swal.fire('Error', error.message, 'error');
