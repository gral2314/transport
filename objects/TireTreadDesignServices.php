...
class TireTreadDesignServices
{
    public function list(array $filters = []): array
    {
        try {
            $query = TireTreadDesign::find();
    ...
    {
        $model = TireTreadDesign::findOne(['code' => $pk]);
        if ($model === null) {
            return ['Success' => 'Error', 'Msg' => 'Diseño de rodada no encontrado', 'Data' => []];
        }
    ...
    public function save(array $data, ?Transaction $transaction = null): array
    {
        ...
        try {
            $pk = $data['code'] ?? null;
            $model = ($pk !== null) ? TireTreadDesign::findOne(['code' => $pk]) : null;
            if ($model === null) {
                $model = new TireTreadDesign();
            }
            ...
            return ['Success' => 'Ok', 'Msg' => 'Diseño de rodada guardado', 'Data' => $model->toArray()];
        } catch (\Throwable $e) {
            if ($ownTx) {
                $tx->rollBack();
            }
    ...
    public function delete(string $pk, ?Transaction $transaction = null): array
    {
        ...
        try {
            ...
            return ['Success' => 'Ok', 'Msg' => 'Diseño de rodada eliminado', 'Data' => []];
        } catch (\Throwable $e) {
            if ($ownTx) {
                $tx->rollBack();
            }
    ...
    public function getFormOptions(): array
    {
        return ['Success' => 'Ok', 'Msg' => '', 'Data' => [
            'active_options' => TireTreadDesign::getActiveOptions(),
        ]];
    }
}
