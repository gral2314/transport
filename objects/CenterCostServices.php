...
class CenterCostServices
{
    public function list(array $filters = []): array
    {
        try {
            $query = CenterCost::find();
    ...
    {
        $model = CenterCost::findOne(['code' => $pk]);
        if ($model === null) {
    ...
    public function save(array $data, ?Transaction $transaction = null): array
    {
        ...
        try {
            $pk = $data['code'] ?? null;
            $model = ($pk !== null) ? CenterCost::findOne(['code' => $pk]) : null;
            if ($model === null) {
    ...
    public function getFormOptions(): array
    {
        return ['Success' => 'Ok', 'Msg' => '', 'Data' => [
            'active_options' => CenterCost::getActiveOptions(),
        ]];
    }
}
