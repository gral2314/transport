...
class PaymentMethodsServices 
{
    public function list(array $filters = []): array
    {
        try {
            ...
            $msg = 'Error al listar métodos de pago';
    ...
    {
        $m = PaymentMethods::findOne(['code' => $pk]);
        if ($m === null) return ['Success' => 'Error', 'Msg' => 'Método no encontrado', 'Data' => []];
    ...
    public function save(array $data, ?Transaction $transaction = null): array
    {
        ...
        try {
            $pk = $data['code'] ?? null;
            $m = ($pk !== null) ? PaymentMethods::findOne(['code' => $pk]) : null;
            if ($m === null) $m = new PaymentMethods();
            ...
            if ($ownTx) $tx->commit();
            return ['Success' => 'Ok', 'Msg' => 'Método guardado', 'Data' => $m->toArray()];
        } catch (\Throwable $e) {
            if ($ownTx) $tx->rollBack();
            \Yii::error($e->getMessage(), __METHOD__);
            $msg = 'Error al guardar método de pago';
            if (YII_DEBUG) $msg .= ': ' . $e->getMessage();
            return ['Success' => 'Error', 'Msg' => $msg, 'Data' => []];
        }
    }
    public function delete(string $pk, ?Transaction $transaction = null): array
    {
        ...
        try {
            ...
            if ($m === null) {
                if ($ownTx) $tx->rollBack();
                return ['Success' => 'Error', 'Msg' => 'Método no encontrado', 'Data' => []];
            }
            $m->delete();
            if ($ownTx) $tx->commit();
            return ['Success' => 'Ok', 'Msg' => 'Método eliminado', 'Data' => []];
        } catch (\Throwable $e) {
            if ($ownTx) $tx->rollBack();
            \Yii::error($e->getMessage(), __METHOD__);
            $msg = 'Error al eliminar método';
            if (YII_DEBUG) $msg .= ': ' . $e->getMessage();
            return ['Success' => 'Error', 'Msg' => $msg, 'Data' => []];
        }
    }
