...
class DocTypeVehiculeServices
{
    public function list(array $filters = []): array
    {
        try {
            $query = DocTypeVehicule::find();
    ...
    {
        $model = DocTypeVehicule::findOne(['code' => $pk]);
        if ($model === null) {
    ...
    public function save(array $data, ?Transaction $transaction = null): array
    {
        ...
        try {
            $pk = $data['code'] ?? null;
            $model = ($pk !== null) ? DocTypeVehicule::findOne(['code' => $pk]) : null;
            if ($model === null) {
                $model = new DocTypeVehicule();
            }
    ...
    public function getFormOptions(): array
    {
        return ['Success' => 'Ok', 'Msg' => '', 'Data' => [
            'active_options' => DocTypeVehicule::getActiveOptions(),
        ]];
    }
}
