...
class CargoTypeServices
{
    public function list(array $filters = []): array
    {
        try {
            $query = CargoType::find();
    ...
    {
        $model = CargoType::findOne(['code' => $pk]);
        if ($model === null) {
    ...
    public function save(array $data, ?Transaction $transaction = null): array
    {
        ...
        try {
            $pk = $data['code'] ?? null;
            $model = ($pk !== null) ? CargoType::findOne(['code' => $pk]) : null;
            if ($model === null) {
                $model = new CargoType();
            }
    ...
    public function getFormOptions(): array
    {
        return ['Success' => 'Ok', 'Msg' => '', 'Data' => [
            'active_options' => CargoType::getActiveOptions(),
        ]];
    }
}
