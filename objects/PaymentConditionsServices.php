...
class PaymentConditionsServices
{
    public function list(array $filters = []): array
    {
        try {
            $query = PaymentConditions::find();
            ...
            $msg = 'Error al listar condiciones de pago';
    ...
    {
        $m = PaymentConditions::findOne(['code' => $pk]);
        if ($m === null) return ['Success' => 'Error', 'Msg' => 'Condición no encontrada', 'Data' => []];
    ...
    public function save(array $data, ?Transaction $transaction = null): array
    {
        ...
        try {
            $pk = $data['code'] ?? null;
            $m = ($pk !== null) ? PaymentConditions::findOne(['code' => $pk]) : null;
            if ($m === null) $m = new PaymentConditions();
            ...
            if ($ownTx) $tx->commit();
            return ['Success' => 'Ok', 'Msg' => 'Condición guardada', 'Data' => $m->toArray()];
        } catch (\Throwable $e) {
            if ($ownTx) $tx->rollBack();
            \Yii::error($e->getMessage(), __METHOD__);
            $msg = 'Error al guardar condición de pago';
            if (YII_DEBUG) $msg .= ': ' . $e->getMessage();
            return ['Success' => 'Error', 'Msg' => $msg, 'Data' => []];
        }
    }
    public function delete(string $pk, ?Transaction $transaction = null): array
    {
        ...
        try {
            ...
            if ($m === null) {
                if ($ownTx) $tx->rollBack();
                return ['Success' => 'Error', 'Msg' => 'Condición no encontrada', 'Data' => []];
            }
            $m->delete();
            if ($ownTx) $tx->commit();
            return ['Success' => 'Ok', 'Msg' => 'Condición eliminada', 'Data' => []];
        } catch (\Throwable $e) {
            if ($ownTx) $tx->rollBack();
            \Yii::error($e->getMessage(), __METHOD__);
            $msg = 'Error al eliminar condición';
            if (YII_DEBUG) $msg .= ': ' . $e->getMessage();
            return ['Success' => 'Error', 'Msg' => $msg, 'Data' => []];
        }
    }
